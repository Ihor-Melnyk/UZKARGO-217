...
function onTaskExecuteEnterResultSpecification(routeStage) {
  debugger;
  if (routeStage.executionResult == "executed") {
    if (!EdocsApi.getAttributeValue("SpesificationResult").value) throw `Внесіть значення в поле "Результат розгляду ТУ"`;
  }
}
...
function onTaskExecuteAddEmployee(routeStage) {
  debugger;
  if (routeStage.executionResult == "executed") {
    if (!EdocsApi.getAttributeValue("Responsible").value) throw `Не заповнено значення поля «Відповідальний працівник»`;
  }
}
...
function onTaskExecuteSendOutDoc(routeStage) {
  debugger;
  if (routeStage.executionResult == "executed") {
    if (!EdocsApi.getAttributeValue("VisaHolder").value) throw `Не заповнено значення поля "Інфориаація щодо Технічних умов"`;
